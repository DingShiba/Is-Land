  ...
  <template>
    <div class="marqueeBox" :style="{height: `${rowsNum * lineHeight}px`, overflow: 'hidden'}">
      <ul :style="translate">
        <li v-for="(item, index) in msgList" style="width: 100%;height: 40px" :style="{'line-height': `${lineHeight}px`}" :key="index">{{item}}</li>
      </ul>
    </div>
  </template>
      ...
      created() {
        ...
        setInterval(() => {
          ...
          this.translate = `transform: translate3d(0px, ${
            this.top
          }px, 0px); transition: transform 1000ms ease 0s`;
